...
/*
bool sendRectangle(adhoc_communication::SendRectangle::Request &req, adhoc_communication::SendRectangle::Response &res){
	ROS_DEBUG("Service Called to send a Rect...");
	string s_msg = getSerializedMessage(req.rect);
	res.status = sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_RECTANGLE, req.topic);
	return res.status;
}*/
/*
...
bool sendAnything(adhoc_communication::SendAnything::Request &req, adhoc_communication::SendAnything::Response &res){
    ROS_DEBUG("Service Called to send anything...");
    res.status = sendPacket(req.dst_robot, req.serialized_data, req.type, req.topic);
    return res.status;
}
...
int main(int argc, char **argv)
{
    ...
    ros::ServiceServer sendRobotUpdateS = n_pub->advertiseService(robot_prefix + node_prefix + "send_robot_update", sendRobotUpdate);
    ros::ServiceServer sendTwistS = n_pub->advertiseService(robot_prefix + node_prefix + "send_twist", sendTwist);
    ...
    /*Tutorial*/
    //ros::ServiceServer sendRectangleS = n_pub->advertiseService("send_rectangle", sendRectangle);
    ros::ServiceServer sendAnythingService = n_pub->advertiseService("send_anything", sendAnything);
    /*INIT THREADS*/
...
//#endif
        	//ROS_INFO("Beacon Thread in while");
    ...
    if (req.hostname_source_.compare(hostname) != 0 && (req.header_.hop_count + 1 <= req.header_.hop_limit || req.header_.hop_limit == 0) && !route_req_exsits)//(!routeReqestExsists || req.mc_flag_))
    {
        if (req.mc_flag_ == false)
        {
            ...
            if (req.hostname_destination_.compare(hostname) == 0)
            {
                ...
                ROS_INFO("ADD ROUTE: ID[%u] SOURCE[%s] DEST[%s] NEXT HOP[%s] PREVIUS HOP[%s] PATH:[%s]", 
                    new_route.id, new_route.hostname_source.c_str(), new_route.hostname_destination.c_str(), getMacAsStr(new_route.next_hop).c_str(), getMacAsStr(new_route.previous_hop).c_str(), getPathAsStr(rr.path_l_).c_str());
...
void publishPacket(Packet * p)
{
    ...
    if (p->mc_group_.compare("") == 0 || mc_member)
    {
        ...
        try
        {
			...
			ROS_INFO("PACKET TOPIC [%s]", p->topic_.c_str());
			ROS_INFO("PACKET frame data type [%d]", p->data_type_);
            ...
            else if (p->data_type_ == FRAME_DATA_TYPE_ROBOT_UPDATE)
            {
            ...
            else if (p->data_type_ == FRAME_DATA_TYPE_RECTANGLE){
                adhoc_communication::Rectangle rect;
                ROS_INFO("%s", payload.c_str());
                ...
                */
                // Implement own messagetypes here
